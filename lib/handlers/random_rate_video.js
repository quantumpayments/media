...
var debug = require('debug')('qpm_media:random_rate_video')
var fs = require('fs')
...
function handler(req, res) {
  ...
  var type        = req.query.type || 'getRandomVideo'
 ...
 } else if ( type === 'getRandomVideo' ) {
   cost = 25
   fn = qpm_media.getRandomVideo
 ...
 } else if ( type === 'getTaggedFragment' ) {
   cost = 40
      ...
      if (balance > cost) {
        ...
        wc.insert(credit, res.locals.sequelize, res.locals.config, function(err, ret) {
          if (err) {
            ...
            fn(params, config, conn).then(function(ret){
              if (err) {
                console.error(err)
              } else {
                ...
                if (ret.ret && ret.ret[0] && ret.ret[0][0] && ret.ret[0][0].uri) {
                  ...
                  res.render('pages/media/random_rate_video', { ui : config.ui })
                  return
                }
                debug(media)
