...
function updateRating(rating, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = 'UPDATE Rating set rating = :rating where reviewer_id = :reviewer and uri = :uri'
...
function getRandomUnseenFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var webidClause = ''
    if (params.webid) {
      webidJoins = ' left join User u on u.id = f.user_id '
      webidJoins += ' and u.uri = :webid '
    }

    var sql = "select a.uri, a.id id, f.end, contentType from Media a left join Fragment f on a.id = f.id " + tagJoins + webidJoins + " where contentType = 'video' " + tagClause + " and f.id is NULL order by RAND() limit 1"
...
function getLastSeen(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    conn.query(sql, { replacements: {  } }).then(function(frag){
      var sql = "select * from Rating order by datePublished desc LIMIT 1;"
      debug(frag)
      val = frag
