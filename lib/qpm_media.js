...
function getLastFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = "select a.uri, a.id id, f.end, a.contentType, me.subtitlesURI, me.charenc, a.safe media_safe from Media a left join Fragment f on a.id = f.id left join Meta me on a.id = me.id " + tagJoins + webidJoins + " where contentType = 'video' " + safeClause + tagClause + webidClause + " order by f.lastSeen desc limit 1;"
...
function getRatedFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = "select m.length, a.uri, a.id, max(f.end) end, a.contentType, max(f.rating) mrat, m.subtitlesURI, m.charenc from Media a join Fragment f on  a.id = f.id join Meta m on m.id = a.id " + webidJoins + tagJoins + " where contentType = 'video' " + webidClause + tagClause + "  " + safeClause + " group by f.id having mrat >= 9 and m.length >  end  order by RAND() limit 1;"
