...
function addFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql
    if (params.id) {
      sql = 'INSERT into Fragment values (:id, :start, :end, NULL, NOW())'
    } else {
      sql = 'INSERT into Fragment Select m.id, :start, :end, NULL, NOW() from Media m where m.uri = :uri'
    }
    debug(sql)

    conn.query(sql, { replacements: { "id" : params.id, "start" : params.start, "end" : params.end, "uri" : params.uri} }).then(function(ret){
      return resolve({"ret" : ret, "conn" : conn})
    }).catch(function(err) {
      return reject({"err" : err, "conn" : conn})
    })
...
function updateLastSeen(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    conn.query(sql, { replacements: { "uri" : params.uri, "cacheURI" : params.cacheURI } }).then(function(ret){
      return resolve({"ret" : ret, "conn" : conn})
    }).catch(function(err) {
      return reject({"err" : err, "conn" : conn})
    })
...
function getRandomUnseenFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (params.tag) {
      var sql = "select a.uri, a.id id, contentType from Media a left join Fragment f on a.id = f.id join MediaTag mt on mt.media_id = a.id join Tag t on t.id = mt.tag_id where tag = :tag and contentType = 'video' and f.id is null order by RAND() limit 1"
    } else {
...
function getLastFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (params.tag) {
      var sql = "select a.uri, a.id id, f.end, contentType from Media a left join Fragment f on a.id = f.id join MediaTag mt on mt.media_id = a.id join Tag t on t.id = mt.tag_id where tag = :tag and contentType = 'video' order by f.lastSeen desc limit 1"
...
function getRatedFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = "select a.uri, a.id, max(f.end) end, a.contentType, max(f.rating) mrat from Media a , Fragment f, Meta m where a.id = f.id and m.id = a.id and contentType = 'video' and end < m.length group by f.id having mrat >= 9 order by RAND()  limit 1"
