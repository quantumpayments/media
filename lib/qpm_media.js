...
function addMeta(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = 'INSERT into Meta select a.id, :length, :subtitlesURI from Media a where a.uri = :uri ;'
    debug('addMeta', sql, params)

    conn.query(sql, { replacements: { "length" : params.length, "subtitlesURI": params.subtitlesURI, "uri" : params.uri } }).then(function(ret){
      return resolve({"ret" : ret, "conn" : conn})
    }).catch(function(err) {
      return reject({"err" : err, "conn" : conn})
    })
...
function getRandomUnseenImage(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (params.reviewer) {
      var sql = "SELECT * from Media m where m.uri not in (select r.uri from Rating r join User u on u.id = r.reviewer_id) and contentType = 'image' and lastSeen is NULL " + safeClause + " and FLOOR( m.id / :optimization ) = FLOOR( RAND() * :optimization ) LIMIT 1;"
    } else {
...
function getRandomUnseenFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = "select a.uri, a.id id, f1.end, a.contentType from Media a left join Fragment f1 on f1.id = a.id " + tagJoins + " where a.contentType = 'video' " + tagClause + " and a.id not in (select f2.id from Fragment f2 join User u on u.id = f2.user_id " + webidClause + " ) " + safeClause + " order by RAND() limit 1;"
...
function getLastFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = "select a.uri, a.id id, f.end, a.contentType, me.subtitlesURI, a.safe media_safe, u.safe user_safe from Media a left join Fragment f on a.id = f.id left join Meta me on a.id = me.id " + tagJoins + webidJoins + " where contentType = 'video' " + safeClause + tagClause + webidClause + " order by f.lastSeen desc limit 1;"
...
function getRatedFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = "select a.uri, a.id, max(f.end) end, a.contentType, max(f.rating) mrat from Media a join Fragment f on  a.id = f.id join Meta m on m.id = a.id " + webidJoins + tagJoins + " where contentType = 'video' " + webidClause + tagClause + "   and end < m.length " + safeClause + " group by f.id having mrat >= 9 order by RAND() limit 1;"
...
function getTaggedFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = "select a.uri, a.id, max(f.end) end, a.contentType, max(f.rating) mrat from Media a, Fragment f, Meta m, Tag t, MediaTag mt where a.id = f.id and m.id = a.id and t.tag = :tag and t.id = mt.tag_id and a.id = mt.media_id and contentType = 'video' and end < m.length " + safeClause + " group by f.id order by RAND() limit 1"
