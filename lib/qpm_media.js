...
function getLastFragment(params, config, conn) {
  ...
  return new Promise((resolve, reject) => {
    ...
    var sql = "select a.uri, a.id id, f.end, a.contentType, me.subtitlesURI from Media a left join Fragment f on a.id = f.id left join Meta me on a.id = me.id " + tagJoins + webidJoins + " where contentType = 'video' " + safeClause + tagClause + webidClause + " order by f.lastSeen desc limit 1;"
